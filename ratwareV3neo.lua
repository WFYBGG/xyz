...
-- [[ Movement exclusivity & state hooks ]]
pcall(function()
    if Toggles and Toggles.SpeedhackToggle then
        Toggles.SpeedhackToggle:OnChanged(function(v)
            if v then
                if Toggles.FlightToggle and Toggles.FlightToggle.Value then Toggles.FlightToggle:SetValue(false) end
                if Toggles.AttachtobackToggle and Toggles.AttachtobackToggle.Value then Toggles.AttachtobackToggle:SetValue(false) end
                MovementState.set("speedhack", true)
            else
                MovementState.set("speedhack", false)
            end
        end)
    end
    if Toggles and Toggles.FlightToggle then
        Toggles.FlightToggle:OnChanged(function(v)
            if v then
                if Toggles.SpeedhackToggle and Toggles.SpeedhackToggle.Value then Toggles.SpeedhackToggle:SetValue(false) end
                if Toggles.AttachtobackToggle and Toggles.AttachtobackToggle.Value then Toggles.AttachtobackToggle:SetValue(false) end
                MovementState.set("fly", true)
            else
                MovementState.set("fly", false)
            end
        end)
    end
    if Toggles and Toggles.AttachtobackToggle then
        Toggles.AttachtobackToggle:OnChanged(function(v)
            if v then
                if Toggles.FlightToggle and Toggles.FlightToggle.Value then Toggles.FlightToggle:SetValue(false) end
                if Toggles.SpeedhackToggle and Toggles.SpeedhackToggle.Value then Toggles.SpeedhackToggle:SetValue(false) end
                MovementState.set("atb", true)
            else
                MovementState.set("atb", false)
            end
        end)
    end
end)
...
-- [[ MovementState Manager ]] 
local MovementState = (function()
    local state = {
        active = { speedhack=false, fly=false, tween=false, atb=false },
        original = { JumpPower = nil, WalkSpeed = nil },
        _charConns = {},
    }
    ...
    local function onCharacterAdded(char)
        state.original.JumpPower = nil
        state.original.WalkSpeed = nil
        task.defer(function()
            local hum = getHumanoid()
            if not hum then return end
            if state.active.speedhack or state.active.fly or state.active.tween or state.active.atb then
                if state.original.JumpPower == nil then state.original.JumpPower = hum.JumpPower end
                if state.original.WalkSpeed == nil then state.original.WalkSpeed = hum.WalkSpeed end
                hum-- JumpPower handled by MovementState
            end
        end)
    end

    if LocalPlayer.Character then onCharacterAdded(LocalPlayer.Character) end
    table.insert(state._charConns, LocalPlayer.CharacterAdded:Connect(onCharacterAdded))

    local function apply()
        local hum = getHumanoid()
        if not hum then return end
        local anyActive = false
        for _,v in pairs(state.active) do if v then anyActive = true break end end
        if anyActive then
            if state.original.JumpPower == nil then state.original.JumpPower = hum.JumpPower end
            if state.original.WalkSpeed == nil then state.original.WalkSpeed = hum.WalkSpeed end
            hum-- JumpPower handled by MovementState
        else
            if state.original.JumpPower ~= nil then hum.JumpPower = state.original.JumpPower end
            if state.original.WalkSpeed ~= nil then hum.WalkSpeed = state.original.WalkSpeed end
            state.original.JumpPower = nil
            state.original.WalkSpeed = nil
        end
    end

    local priorities = { speedhack=1, fly=2, atb=3, tween=4 } -- higher number = higher priority

    local api = {}

    function api.set(flag, value)
        if state.active[flag] == value then return end
        state.active[flag] = value and true or false

        if value then
            local myp = priorities[flag] or 0
            for k,_ in pairs(state.active) do
                if k ~= flag and state.active[k] and (priorities[k] or 0) < myp then
                    pcall(function()
                        if k == "speedhack" and Toggles.SpeedhackToggle then Toggles.SpeedhackToggle:SetValue(false) end
                        if k == "fly" and Toggles.FlightToggle then Toggles.FlightToggle:SetValue(false) end
                        if k == "atb" and Toggles.AttachtobackToggle then Toggles.AttachtobackToggle:SetValue(false) end
                        if k == "tween" and _G.StopTween then _G.StopTween() end
                    end)
    ...
    function api.anyActive()
        for _,v in pairs(state.active) do if v then return true end end
        return false
    end
...
char.Humanoid-- JumpPower handled by MovementState
            end
        end
    end)
    ...
    player.CharacterAdded:Connect(function(character)
        pcall(function()
            ...
            if Toggles.SpeedhackToggle.Value then
                ...
                character.Humanoid-- JumpPower handled by MovementState
            end
        end)
    end)
...
pcall(function()
    ...
    local function recordPart(part)
        if part and part:IsA("BasePart") and originalStates[part] == nil then
            local ok, val = pcall(function() return part.CanCollide end)
            if ok then originalStates[part] = val end
        end
    end
    ...
    local function enableNoclip()
        ...
        local char = player.Character
        if char then
            for _, d in ipairs(char:GetDescendants()) do recordPart(d) end
            descAddedConn = char.DescendantAdded:Connect(function(obj)
    ...
    rs.RenderStepped:Connect(function(delta)
        pcall(function()
            ...
            if flyEnabled and character and humanoid and hrp then
                ...
                humanoid-- JumpPower handled by MovementState
...
local function tweenToBack()
    ...
    local success, result = pcall(function()
        ...
        local function createTween()
            local backGoal = targetHrp.CFrame * CFrame.new(0, Options.ATBHeight.Value, Options.ATBDistance.Value) -- Dynamic adjustment
...
local function startAttach()
    local success, result = pcall(function()
        ...
        attachConn = RunService.RenderStepped:Connect(function()
            ...
            if isLocked then
                hrp.CFrame = targetHrp.CFrame * CFrame.new(0, Options.ATBHeight.Value, Options.ATBDistance.Value) -- Dynamic adjustment
