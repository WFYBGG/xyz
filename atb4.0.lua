...
-- Move to target: Use interpolation instead of tween for dynamic movement
local moveSpeed = 150 -- Studs per second
local function moveToBack()
    ...
    enableNofall()
    isTweening = false
    isLocked = true -- Lock for continuous attachment
end
...
local function startAttach()
    ...
    moveToBack() -- Start moving to target
    attachConn = RunService.RenderStepped:Connect(function(deltaTime)
        ...
        if not isLocked then
            ...
            local targetPos = backGoal.Position
            local moveDir = (targetPos - currentPos).Unit * step
            local newPos = currentPos + moveDir
            distance = (newPos - targetPos).Magnitude
